...
Pipeline: parse header -> look up node key -> AEAD open -> replay check ->
store -> publish MQTT -> ACK (alerts only).
"""
...
class Ingest:
    ...
    def handle_frame(self, frame: bytes, rssi=None, snr=None):
        ...
        with self._lock:
            ...
            try:
                ...
                duplicate = True
            self.db.save_replay(hdr.node_id, rw.to_state())

        if duplicate:
            self.db.add_raw(hexf, rssi, snr, "replay")
            # Blind retransmit of an alert we already have: re-ACK so the
            # node stops retransmitting, but store/publish nothing.
            if hdr.msg_type == proto.MSG_ALERT:
                self._send_ack(key, hdr, proto.ACK_DUP)
            return

        self.db.add_raw(hexf, rssi, snr, "ok")
        if hdr.msg_type == proto.MSG_ALERT:
            self._on_alert(key, hdr, proto.Alert.unpack(payload), rssi, snr)
        elif hdr.msg_type == proto.MSG_HEARTBEAT:
            self._on_heartbeat(hdr, proto.Heartbeat.unpack(payload), rssi, snr)
    ...
    def _send_ack(self, key: bytes, hdr: proto.Header, status: int):
        ...
        ack_hdr = proto.Header(self.net_id, proto.MSG_ACK, hdr.node_id, hdr.seq)
    ...
    def _on_alert(self, key, hdr, a: proto.Alert, rssi, snr):
        ...
        self._send_ack(key, hdr, proto.ACK_OK)
