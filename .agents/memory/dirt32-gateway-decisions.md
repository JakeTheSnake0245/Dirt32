...
- **Bridge serial framing (hard-won):** `getPacketLength(false)` on SX126x is NOT cleared after `readData` — register holds stale value on next loop tick causing double-fire. Fixed with DIO1 interrupt flag (`volatile bool rxDone`). `radio.transmit()` fires TX_DONE on DIO1 too; must `rxDone = false` after transmit before `startReceive()`. Build entire RX line with `snprintf` + single `Serial.print(line)` — multiple `print()` calls split across USB CDC packets causing orphaned "RX " prefixes.
